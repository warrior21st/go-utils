...
func Base58Encode(input []byte) string {
	return Encode(input, BitcoinAlphabet)
}

// Base58Decode 解码
func Base58Decode(input []byte) string {
	res, err := Decode(string(input), BitcoinAlphabet)
	if err != nil {
		panic(err)
	}
	return string(res)
...
// Base58Decode 解码，返回[]byte
func Base58DecodeToBytes(input []byte) []byte {
	res, err := Decode(string(input), BitcoinAlphabet)
	if err != nil {
		panic(err)
	}
