...
//获取请求referer
func GetClientReferer(req *http.Request) string {
	r := req.Header.Get("Referer")
	if commonutil.IsNilOrWhiteSpace(r) {
		r = req.Header.Get("referer")
	}
	if commonutil.IsNilOrWhiteSpace(r) {
		r = req.Header.Get("REFERER")
...
//读取请求头中的参数
func ReadHttpRequestHeaderValue(req *http.Request, key string) string {
